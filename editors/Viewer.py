...
from graphics.DataContainer import DataContainer
            ...
            if values[1] == "debug":
                ...
                if pou_name == values[0]:
                    ...
                    if blockname.upper() in [name.upper() for name in self.ParentWindow.Controler.GetProjectPouNames(self.ParentWindow.Debug)]:
                        ...
                        self.ParentWindow.ParentWindow.RefreshPouInstancesViewerPanel()
...
class Viewer(EditorPanel):
    ...
    def AddBlockPinMenuItems(self, menu, connector):
        ...
        type = self.Controler.GetEditedElementType(self.TagName, True)
    ...
    def RefreshView(self, variablepanel=True, selection=None):
        ...
        instances = self.Controler.GetEditedElementInstancesInfos(self.TagName, debug=False)
    ...
    def IsOfType(self, type, reference):
        return self.Controler.IsOfType(type, reference, False)

    def IsEndType(self, type):
        return self.Controler.IsEndType(type)

    def GetBlockType(self, type, inputs=None):
        return self.Controler.GetBlockType(type, inputs, False)
    ...
    def PopupBlockMenu(self, connector=None):
        ...
        if connector is not None and connector.IsCompatible("BOOL"):
            ...
            edit = self.SelectedElement.GetType() in self.Controler.GetProjectPouNames(False)
    ...
    def OnViewerLeftDown(self, event):
        ...
        if self.Mode == MODE_SELECTION:
            ...
            if event.ShiftDown() and not event.ControlDown() and self.SelectedElement is not None:
                ...
                elif connector is not None and not event.ControlDown():
            ...
            if event.ControlDown() and not event.ShiftDown():
                if not isinstance(self.SelectedElement, Graphic_Group):
                    if isinstance(self.SelectedElement, FBD_Block):
                        ...
                        instance_type = None
                    if instance_type in self.Controler.GetProjectPouNames(False):
        ...
        if event.MiddleIsDown() or self.Mode == MODE_MOTION:
            ...
            if self.rubberBand.IsShown():
                ...
                if self.DrawingWire:
                    ...
                    if movex != 0 or movey != 0:
                        self.RefreshRect(self.GetScrolledRect(self.SelectedElement.GetRedrawRect(movex, movey)), False)
            elif self.StartMousePos is not None and event.Dragging():
    ...
    def OnChar(self, event):
        ...
        if self.Scaling is not None:
            ...
            scaling = (8, 8)
        if keycode == wx.WXK_DELETE and self.SelectedElement is not None:
            ...
            wx.CallAfter(self.SetCurrentCursor, 0)
            self.RefreshRect(self.GetScrolledRect(rect), False)
        elif keycode == wx.WXK_RETURN and self.SelectedElement is not None:
            self.SelectedElement.OnLeftDClick(event, self.GetLogicalDC(), self.Scaling)
    ...
    def AddNewBlock(self, bbox, wire=None):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.RefreshVariablePanel()
            self.ParentWindow.RefreshPouInstancesViewerPanel()
        dialog.Destroy()
    ...
    def AddNewActionBlock(self, bbox, wire=None):
        ...
        dialog.SetActionList(self.Controler.GetEditedElementActions(self.TagName, False))
        dialog.SetVariableList(self.Controler.GetEditedElementInterfaceVars(self.TagName, debug=False))
        if dialog.ShowModal() == wx.ID_OK:
            id = self.GetNewId()
    ...
    def DeleteBlock(self, block):
        ...
        wx.CallAfter(self.ParentWindow.RefreshPouInstancesViewerPanel)
    ...
    def Cut(self):
        if (self.IsBlock(self.SelectedElement) or self.IsComment(self.SelectedElement) or isinstance(self.SelectedElement, Graphic_Group)):
            blocks, wires = self.SelectedElement.GetDefinition()
            text = self.Controler.GetEditedElementInstancesCopy(self.TagName, blocks, wires, False)
    ...
    def Paste(self, bbx=None):
        ...
        if bbx is None:
            ...
            middle = True
            new_pos = [bbx.x, bbx.y]
        result = self.Controler.PasteEditedElementInstances(self.TagName, element, new_pos, middle, False)
    ...
    def Find(self, direction, search_params):
        if self.SearchParams != search_params:
            ...
            self.SearchResults = []
            blocks = []
            for infos, start, end, _text in self.Controler.SearchInPou(self.TagName, search_params, False):
