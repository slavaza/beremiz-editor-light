...
class PouInstancesViewerPanel(wx.Panel):

    def __init__(self, parent, window, controller):
        ...
        self.DebugInstanceImage = self.TreeRightImageList.Add(GetBitmap("unknown"))
    ...
    def RefreshView(self):
        ...
        if self.Controller is not None and self.PouTagName is not None:
            if self.PouTagName.split('::')[0] in ['A', 'T']:
                self.PouInfos = self.Controller.GetPouVariables('P::%s' % self.PouTagName.split('::')[1], False)
            else:
                self.PouInfos = self.Controller.GetPouVariables(self.PouTagName, False)
            if None in self.Controller.GetEditedElementType(self.PouTagName, False) and self.PouInfos is not None:
        ...
        if self.PouInfos is not None:
            ...
            for var_infos in self.PouInfos.variables:
                ...
                # crutch for derived variables bag
                if (var_infos.var_class == ''): var_infos.var_class = 10
                # earth
    ...
    def RefreshInstanceChoice(self):
        ...
        if self.Controller is not None and self.PouInfos is not None:
            instances = self.Controller.SearchPouInstances(self.PouTagName, False)
    ...
    def DebugButtonCallback(self, infos):
        if self.InstanceChoice.GetSelection() != -1:
            ...
            if not var_class in [ITEM_TRANSITION, ITEM_ACTION]:
