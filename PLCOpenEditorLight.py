...
from PLCEditorFrame import PLCEditorFrame, AppendMenu
from PLCEditorFrame import \
    TITLE, \
    EDITORTOOLBAR, \
    FILEMENU, \
    EDITMENU, \
...
class PLCOpenEditor(PLCEditorFrame):
    ...
    def _init_coll_FileMenu_Items(self, parent):
        ...
        self.Bind(wx.EVT_MENU, self.OnGenerateProgramMenu, id=ID_PLCOPENEDITORFILEMENUGENERATE)
    ...
    def __init__(self, parent, fileOpen=None):
        ...
        PLCEditorFrame.__init__(self, parent)
        ...
        if fileOpen is not None:
            ...
            if os.path.isfile(fileOpen):
                ...
                self.PouInstancesViewerPanel.SetController(controler)
                self._Refresh(PROJECTTREE, POUINSTANCESVIEWERPANEL, LIBRARYTREE)
    ...
    def OnCloseProjectMenu(self, event):
        if self.CheckSaveBeforeClosing(): 
            self.ResetView()
            self._Refresh(TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU)
    ...
    def OnViewGeneratedProgram(self, event):
        if self.Controler:
            result = self.Controler.GetProgramFilePath()
        else: 
            result = ''
        ...
        if not result:
            dialog = wx.FileDialog(self, _("Choose a file"), os.getcwd(), os.path.basename('./'),  
                                    _("ST files (*.st)|*.st|All files|*.*"), wx.SAVE | wx.CHANGE_DIR)
            if dialog.ShowModal() != wx.ID_OK:
                return
            self.ViewGeneratedProgram(dialog.GetPath())
        else:
            self.ViewGeneratedProgram(result)

    def GenerateProgramAs(self):
        dialog = wx.FileDialog(self, _("Choose a file"), os.getcwd(), 
                                os.path.basename(self.Controler.GetProgramFilePath()),  
                                _("ST files (*.st)|*.st|All files|*.*"), wx.SAVE | wx.CHANGE_DIR)
        if dialog.ShowModal() == wx.ID_OK:
    ...
    def OnAboutMenu(self, event):
        ...
        info.Description = _("PLCOpenEditorLight is subset of Beremiz project.\n\n"
